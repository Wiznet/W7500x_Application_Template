...
int main(void)
{
	...
	// 1. If MAC address does not exist.				-> input and sace MAC address
	...
	// 2. If Firmware update flag is SET  				-> Erase the entire area, update the firmware as muich as the Fwup_size, app backup (flash) >> app main (flash)
	if(dev_config_e->firmware_update.fwup_flag == SET)
	{
		...
		if(ret == DEVICE_FWUP_RET_SUCCESS)
		{
			...
			/* Set the device working mode: OPEN */
...
//    }
	...
	if(appjump_enable == ON)
	{
		...
		if (dev_config->network_connection[0].working_state == ST_BOOT) // for AppBoot cmd
		{
			appjump_enable = OFF;
			dev_config->network_connection[0].working_state = ST_OPEN;
			dev_config->network_connection[1].working_state = ST_OPEN;
...
/**
  * @brief  Functions for MAC address 
  * @param  None
  * @retval None
  */
uint8_t check_mac_address(void)
...
/**
  * @brief  Functions for Interrupt Vector Table Remap
  * @param  None
  * @retval None
  */
void Copy_Interrupt_VectorTable(uint32_t start_addr)
